...
2. 분석 에이전트의 툴
 ...
 작동 로직: 학습된 회귀 모델을 실행하여 해당 직업군에서 이 정도 내실을 갖췄을 때 나와야 하는 기대 전투력($Expected CP$)을 산출합니다.
  데
 ...
 입력데이터: 기대 전투력(툴 1의 결과) 및 현재 전투력 (Open API 실측 데이터).  
...
## calculator
1. 계산 에이전트의 목적
특정 장비 아이템의 효용성을 알기위하여 그 장비 아이템을 장착했을시 전투력의 상승을 계산하여 출력하는 에이전트이다.

2. 계산 에이전트의 툴
  1) 장비 스탯 계산툴
        입력 데이터: 현재 전투력, 특정 장비의 추가 스탯

        작동 로직: 현재 전투력+ 장비 스탯 환산 전투력을 합산하는 로직이다. 
